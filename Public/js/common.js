console.log('dapps-marketplace-nfts-iotex 1.0');

//const ROOT  = window.location.hostname=='localhost'?'':'/dapps-market-place-nfts-to-iotex';
let Torus = null;
...
function addressToOne(address) {
    if(address.startsWith('0x')){
        return Torus.crypto.getAddress(address).bech32.toLowerCase();
    }
...
function addressToHex(address) {
    if(address.startsWith('')){
        return Torus.crypto.getAddress(address).checksum.toLowerCase();
    }
    return address.toLowerCase();
}
...
async function onBuyConfirm() {
	...
	try {
		...
		wei = new Torus.utils.Unit(item.saleprice).asOne().toWei();
...
async function onResellConfirm() {
	...
	try {
		...
		ctr = await Davinci.contract(abi, adr);
		ctr.wallet = Davinci.wallet;
		wei = new Torus.utils.Unit(order.sellprice).asOne().toWei();
		gas = { gasPrice: config.GASPRICE, gasLimit: config.GASLIMIT, value: wei, from: Davinci.address };
        console.log('Wei', wei.toString());
        console.log('Contract info', orderId, buyer, gas);
	    res = await ctr.methods.buy(orderId, buyer, 1).send(gas);
...
async function onBidConfirm() {
	...
	try {
		...
		ctr = await Davinci.contract(abi, adr);
		ctr.wallet = Davinci.wallet;
		wei = new Torus.utils.Unit(amount).asOne().toWei();
...
async function getTransaction(txid) {
	...
	let tx, rec;
	try	{
		rec = await Torus.getTransactionByHash({txnHash:txid})
		tx = rec.result;
	} catch(ex) {
...
async function sendPayment(receiver, amount, tokenId) {
	let res, txn;
	txn = Torus.transactions.newTx({
        to: receiver,
    });
 
    //Torus.wallet.addByPrivateKey(secret);
    ...
    try { 
        res = await Torus.sendTransaction(signed);
        if(res.error){ 
...
async function isApproved(collection, type='1155', user, operator) {
	...
	try {	
	    adr = collection;
		//ctr = Torus.contracts.createContract(abi, adr);
		ctr = await Davinci.contract(abi, adr);
		//console.log('CTR', ctr);
...
async function startDavinci() {
	...
	setTheme();
	Torus = await torus.login(); // await torus.ethereum.enable()
	await Davinci.init();
