...
function TncLib(){
    ...
    if(chain_id === "4689") {
        ...
        this.IoTeX = new web3.eth.Contract(IoTeXABI, '', {from: this.account});
    ...
    // BINANCE MAINNET
        ...
        this.IoTeX= new web3.eth.Contract( IoTeXABI, 'null', {from:this.account} );
    ...
    this.getFarmMinimum = async function(){
        await sleep(sleep_time);
        return await this.farm.methods.farmFeeMinimum().call({from:this.account});
    };
    ...
    this.newFarm = async function(periodStart, minStake, maxStake, controller, token, uri, preCallback, postCallback, errCallback){
        ...
        await sleep(sleep_time);
        let Balance = web3.utils.toBN(await this.IoTeX.methods.balanceOf(this.account).call({from:this.account}));
        let feeMin = web3.utils.toBN(await this.getFarmMinimum());

        console.log('Wilcard: ', haveWildcard, ' enough : ', Balance.lt( feeMin ) );

        if(!haveWildcard && Balance.lt( feeMin )){
    ...
    this.getPoolMinimum = async function(){
        await sleep(sleep_time);
        return await this.genesis.methods.poolFeeMinimum().call({from:this.account});
    };

    this.newErc1155 = async function(name, ticker, contractJsonUri, proxyRegistryAddress, preCallback, postCallback, errCallback){

        await sleep(sleep_time);
        let  = web3.utils.toBN(await this.IoTeX.methods.balanceOf(this.account).call({from:this.account}));
        let min = web3.utils.toBN(await this.getPoolMinimum());
        ...
        try {
            ...
            gas = await this.genesis.methods.newPool(name, ticker, contractJsonUri, '', proxyRegistryAddress).estimateGas({
                from: this.account,
                value:
                    await this.iHaveAnyWildcard() || IoTeX.gte(min) ? 0 : await this.getPoolFee()
            });
        }catch(e){
            errCallback();
            return;
    ...
    this.setContractURI = async function(erc1155Address, uri, preCallback, postCallback, errCallback){
        ...
        await sleep(sleep_time);
        let balanceOf = await this.IoTeX.methods.balanceOf(this.account).call({from:this.account});
