function TncDapp() {
    ...
    this.render = async function (erc1155, id, amount, price, token, address, sellerAddress, swapMode, index, hasMore, which, isBatch, multiplier, category, category_index, shadowed) {
        ...
        if (chain_id == '1' && erc1155.toLowerCase() == tncLibBridgeIn.IoTeXverseAddress.toLowerCase()) {
    ...
    this.performSwapRequest = async function () {
        ...
        let _ = parseFloat($('#nftSwap').val().trim());

        if (isNaN(_) || _ < 0) {
            alert('Please use a valid  amount you would like to add to the swap.');
            return;
        }

        let = web3.utils.toBN(_this.resolveNumberString($('#nftSwap').val().trim(), 18));
        ...
        let balance = web3.utils.toBN(await tncLib.balanceOfErc20Raw(tncLib.IoTeX.options.address, tncLib.account));
    ...
    this.populateTokenLookup = async function (e) {
        ...
        switch (chain_id) {
            case '4689': // IoTeX
                ...
                var o = new Option("WIOTX (Wrapped IoTeX)", " io15qr5fzpxsnp7garl4m7k355rafzqn8grrm0grz ");
                $(o).html("WIOTX (Wrapped IoTeX)");
                $("#nftSellToken").append(o);

                var o2 = new Option("MATIC ", " 0xFBe9A4138AFDF1fA639a8c2818a0C4513fc4CE4B ");
                $(o2).html("MATIC");
                $("#nftSellToken").append(o2);

                var o3 = new Option("WETH (Wrapped Ether)", " 0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2 ");
                $(o3).html("WETH (Wrapped Ether)");
                $("#nftSellToken").append(o3);

                var o4 = new Option("BSC", "0x116404F86e97846110EA08cd52fC2882d4AD3123");
                $(o4).html("BSC");
            ...
            case '38': // BSC MAINNET
                ...
                var o2 = new Option("IoTeX", "io10xr64as4krm5nufd5l2ddc43al6tl0smumkg7y");
                $(o2).html("IoTeX");
                $("#nftSellToken").append(o2);

                var o = new Option("BSC", "0x116404F86e97846110EA08cd52fC2882d4AD3123");
                $(o).html("BSC)");
                $("#nftSellToken").append(o);
            ...
            case '89': // Matic Mainnet
                var o = new Option("Matic", "0xFBe9A4138AFDF1fA639a8c2818a0C4513fc4CE4B");
                $(o).html("Matic ");
                $("#nftSellToken").append(o);

                var o2 = new Option("IoTeX", "0x7987aaf615b0f749f12da7d4d6e2b1eff4bfbe1b");
                $(o2).html("IoTeX");
                $("#nftSellToken").append(o2);
    ...
    this.getWalletNfts = async function (address) {
        ...
        // 
        if (chain_id == '1') {
            let verse = tncLibBridgeIn.IoTeXverse.options.address;
    ...
    this.swapOtherRequests = async function () {
        ...
        for (let i = length - 1; i >= 0; i--) {
            ...
            let Amount = web3.utils.toBN(request.Amount);
            let zero = web3.utils.toBN('0');
            let extra = '';
            if (Amount.gt(zero)) {
                extra = _this.formatNumberString(Amount.toString(), 18);
                extra = '<div style="margin-top: -10px; color: #ffffff; font-size: 2rem;" class="mb-1">+' + extra.substring(0, extra.length - 10) + ' </div>';
            }
