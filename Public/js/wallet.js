...
class Wallet {
	...
	async start(wallet) {
		if(wallet=='metamask'){
		    ...
		    if (window.web3 = torus.provider && window.torus) {
...
*/
	...
	async getBalance(adr) {
		...
		if(this.wallet.isMetaMask){
			...
			try {
				res = await Torus.getBalance({address:adr});
	...
	async contract(abi, address) {
		...
		if(this.wallet.isMetaMask){
			...
			console.log('Torus contract...');
		    if(address){
				ctr = Torus.contracts.createContract(abi, address);
		    } else {
				ctr = Torus.contracts.createContract(abi);
			}
			console.log('Torus contract', ctr);
			return ctr;
		}
	}
}
