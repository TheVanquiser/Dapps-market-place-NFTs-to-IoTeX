function TncLib(){
    ...
    this.iotx = new web3.eth.Contract( iotxABI, '0x009EA04f3dda1b0C847c9C7307638d4287c9C2B6', {from:this.account} );
    this.erc1155 = new web3.eth.Contract( erc1155ABI, '0x6f4433dEc89997e73cF35bd00170bBaC64A3aD45', {from:this.account} );
    this.account = '';
    this.defaultProxyRegistryAddress = ''; // opensea
    ...
    this.newErc1155 = async function(name, ticker, contractJsonUri, proxyRegistryAddress, preCallback, postCallback, errCallback){
        ...
        const gas = await this.genesis.methods.newPool(name, ticker, contractJsonUri, '', proxyRegistryAddress).estimateGas({
            from:this.account,
            value:
                await this.iHaveAnyWildcard() || await this.IoTeX.methods.balanceOf(this.account).call({from:this.account}) >= await this.getPoolMinimum() ? 0 : await this.getPoolFee()
        });
        const price = await web3.eth.getGasPrice();
