...
function TncLib(){
    ...
    if(chain_id === "4") {
        ...
        this.iotx = new web3.eth.Contract(iotxABI, '0xb93370d549a4351fa52b3f99eb5c252506e5a21e', {from: this.account});
    ...
    // IOTEX TESTNET
        ...
        this.iotx = new web3.eth.Contract(iotxABI, '0x95E20FD5FDd9A8D9D174C7fAE7c3F9eC51BBbF36', {from: this.account});
    ...
    this.getFarmMinimumIotx = async function(){
        await sleep(sleep_time);
        return await this.farm.methods.farmFeeMinimumIotx().call({from:this.account});
    };
    ...
    this.newFarm = async function(periodStart, minStake, maxStake, controller, token, uri, preCallback, postCallback, errCallback){
        ...
        await sleep(sleep_time);
        let iotxBalance = web3.utils.toBN(await this.iotx.methods.balanceOf(this.account).call({from:this.account}));
        let feeMinIotx = web3.utils.toBN(await this.getFarmMinimumIotx());

        console.log('Wilcard: ', haveWildcard, ' enough iotx: ', iotxBalance.lt( feeMinIotx ) );

        if(!haveWildcard && iotxBalance.lt( feeMinIotx )){
    ...
    this.getPoolMinimumiotx = async function(){
        await sleep(sleep_time);
        return await this.genesis.methods.poolFeeMinimumIotx().call({from:this.account});
    };

    this.newErc1155 = async function(name, ticker, contractJsonUri, proxyRegistryAddress, preCallback, postCallback, errCallback){

        await sleep(sleep_time);
        let iotx = web3.utils.toBN(await this.iotx.methods.balanceOf(this.account).call({from:this.account}));
        let feeMinIotx = web3.utils.toBN(await this.getPoolMinimumIotx());
        ...
        try {
            ...
            gas = await this.genesis.methods.newPool(name, ticker, contractJsonUri, '', proxyRegistryAddress).estimateGas({
                from: this.account,
                value:
                    await this.iHaveAnyWildcard() || iotx.gte(minIotx) ? 0 : await this.getPoolFee()
            });
        }catch(e){
            errCallback(e);
            return;
    ...
    this.setContractURI = async function(erc1155Address, uri, preCallback, postCallback, errCallback){
        ...
        await sleep(sleep_time);
        let balanceOf = await this.iotx.methods.balanceOf(this.account).call({from:this.account});
